...
$order_id = $response['outOrderNo'];
$response_sign = $response['sign'];
unset($response['sign']);

if($response['msg'] != 'SUCCESS' || !$order_id || !$response_sign){
	exit;
}
...
if($sign_key){
$sign = create_sign($response, array($sign_key => $app_key));

}else{
	$sign = create_sign($response , $app_key);
}

...
if($response['returncode'] != '00' || !$order_id || !$response_sign){
	exit;
}

$post_data = array (
            'memberid' => $response['memberid'],     //商户号
            'orderid' => $order_id,     //订单号
            'amount' => $response['amount'],     //金额
            'transaction_id' => $response['transaction_id'],     //交易流水号
            'datetime' => $response['datetime'],     //交易时间
            'returncode' => $response['returncode'],     //状态
    );

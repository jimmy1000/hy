...
if($response['status'] != '3' || !$order_id || !$response_sign){
	exit;
}

$post_data = array (
            'merchantId' => $response['merchantId'],     //商户号
            'orderNo' => $order_id,     //订单号
            'orderAmount' => $response['orderAmount'],     //金额
            'paymentDatetime' => $response['paymentDatetime'],     //金额
            'dealId' => $response['dealId'],     //平台订单号
            'status' => $response['status'],     //订单状态
    );

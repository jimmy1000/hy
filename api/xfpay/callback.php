...
if($flag && $res['pay_status'] == 1){
    ...
    if(empty($order)){
        file_put_contents(dirname(__FILE__).'/notfound.log', date('Y-m-d H:i:s')."\tIP:".$ip."提交数据订单不存在错误\t". var_export($res).PHP_EOL , FILE_APPEND);
        die('success'); 
    }
    ...
    if(floatval($order['order_money']*100) == floatval($res['total_fee'])){
        ...
        if(empty($order_)){
         ...
         file_put_contents(dirname(__FILE__).'/money.log', date('Y-m-d H:i:s')."\tIP:".$ip."提交数据金额错误\t". var_export($res).PHP_EOL , FILE_APPEND);
    } 
    die('success');   
}
echo 'success';

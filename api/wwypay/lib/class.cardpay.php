...
函数:
	send()
		...
		调用示例：
			...
			$cardpay->restrict 		= $card_restrict;		//地区限制, 0表示全国范围
			...
			$result	= $cardpay->send();	
	recive()
		接收平台消息，这里会进行身份的验证，如果您自己要做更多后续的处理，请在您自己的程序中进行
		调用示例：
...
class cardpay{
	const card_url		 = 'http://pay.wwypay.com/card/';
	const card_search_url	= 'http://pay.wwypay.com/search.ashx';
	...
	*/
	public function send(){	
		//检查是否正确
		$error 	= 0;
		$msg		= '您调用平台支付接口的参数有误，错误信息如下：';
		if(empty($this->parter)){
			$error 	= 1;
			$msg 	.= '<li>parter不能为空: 商户id，由本平台分配</li>';
		}
		if(empty($this->type)){
			$error 	= 1;
		...
		$url	= cardpay::card_url . "?" . $url . "&sign=" .$sign;
	...
	*/
	public function search($order_id){
		...
		//签名
		$sign	= md5($url. $this->key);
		$url	= cardpay::card_search_url . "?" . $url . "&sign=" .$sign;		
		$result=file_get_contents($url);
		parse_str($result, $output);

...
if(isset($_GET['action'])){
	...
	if($_GET['action'] == 'lock'){
		...
		if(!empty($id)){
		   ...
		   $info = $database->get(DB_PREFIX.'order','*',array('id'=>$id));
			if($info['state'] == "5"){
				message('温馨提示',"当前订单已经是锁定状态,无法进行锁定操作",'my_list.php');
				return;
			}
			if($info['state'] != "0"){
				message('温馨提示',"当前订单已经处理过了,无法进行锁定操作",'my_list.php');
				return;
			}
		...
		if(!empty($id)){
		   ...
		   $info = $database->get(DB_PREFIX.'order','*',array('id'=>$id));
			if($info['state'] == "5"){
				...
				message('温馨提示',"当前订单不是锁定状态,无法进行解锁操作",'my_list.php');
				return;
			}
	...
	if($_GET['action'] == 'confirm'){
		...
		if(!empty($id)){
		   ...
		   $info = $database->get(DB_PREFIX.'order','*',array('id'=>$id));
			if($info['state'] == "5"){
				$database->update(DB_PREFIX."order",array('state'=>1),array('id'=>$id));
				header("Location:my_list.php");
			}else{
				message('温馨提示',"当前订单不是锁定状态,无法进行确认操作",'my_list.php');
				return;
			}
...
if($_POST['ids']) {
 ...
 $tmp = count($_POST['ids']);
 if($tmp <=0) {
	message('温馨提示',"请先选择订单",'my_list.php');
	return;
 }
 $ids = implode(",",$_POST['ids']);
 if($_POST['tmp1']=='111') {
  	 //update vc_order set state=5 WHERE id in(1,2,3,4,5)
  	 $sql="UPDATE vc_order set state=0,lock_id='' WHERE id in(".$ids.") and state=5"; 
 	 $database->query($sql);
	 echo "<script>alert('解锁成功，共处理了".$tmp."条订单');location.href='my_list.php'</script>";
	 exit;
 } else if($_POST['tmp1']=='222') {
	 //update vc_order set state=5 WHERE id in(1,2,3,4,5)
  	 $sql="UPDATE vc_order set state=1 WHERE id in(".$ids.") and state=5"; 
 	 $database->query($sql);
...
$sql_where = " where state = 5 and lock_id = '".$user."' ";
...
if($_POST['ids']) {
 ...
 $ids = implode(",",$_POST['ids']);
 //update vc_order set state=5 WHERE id in(1,2,3,4,5)
 $sql="UPDATE vc_order set state=5,lock_id='".$_SESSION['card_admin']['username']."' WHERE id in(".$ids.")";
 $database->query($sql);
    ...
    <div class="container-fluid">
        <div class="row-fluid">
              <div class="btn-toolbar" style="height:30px;">	
					...
					<form action="my_list.php" method="post" class="form-search pull-right">
					...
					<table class="table table-hover table-bordered">
					  <thead>
                       ...
                       <form action="my_list.php" method="post" name='t2' id='t2'  class="form-search pull-right">
					  <tbody>
					  <?php 

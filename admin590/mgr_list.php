...
if(isset($_GET['action'])){
	...
	if($_GET['action'] == 'lock'){
		...
		if(!empty($id)){
		   ...
		   $info = $database->get(DB_PREFIX.'order','*',array('id'=>$id));
			if($info['state'] == "5"){
				message('温馨提示',"当前订单已经是锁定状态,无法进行锁定操作",'mgr_list.php');
				return;
			}
			if($info['state'] != "0"){
				message('温馨提示',"当前订单已经处理过了,无法进行锁定操作",'mgr_list.php');
				return;
			}
		...
		if(!empty($id)){
		   ...
		   $info = $database->get(DB_PREFIX.'order','*',array('id'=>$id));
			if($info['state'] == "5"){
				...
				message('温馨提示',"当前订单不是锁定状态,无法进行解锁操作",'mgr_list.php');
				return;
			}
	...
	if($_GET['action'] == "del"){
		...
		if(!empty($keywords)){
			$sql_where .= " and (order_id = '".$keywords."' or user_name = '".$keywords."' or pay_order = '".$keywords."' or lock_id = '".$keywords."' or order_desc like '%".$keywords."%' ) ";
		}
		
		$database->query(" delete from ".DB_PREFIX."order ".$sql_where);		
		message('温馨提示',"清空数据成功",'mgr_list.php');
		exit;
	}
}
    ...
    <div class="container-fluid">
        <div class="row-fluid">
              <div class="btn-toolbar" style="height:30px;">	
					...
					<form action="mgr_list.php" method="post" class="form-search pull-right">
					  ...
					  <select name="pay_type" id="pay_type" class="search-query" style="width:120px;margin-right:10px;">
                        ...
                        <option value='QQPAY'>QQ扫码</option>
						<option value='QQWAPPAY'>手机QQ钱包</option>
					  </select>
					  ...
					  <a href="javascript:;" onclick="delForm('mgr_list.php?action=del&<?php echo $extraStr;?>')" class="btn btn-danger">清空数据</a>
					...
					<table class="table table-hover table-bordered">
					  ...
					  <tr>
						  ...
						  if($item['state'] == "5"){
						  ?>
						  <a title='解锁' onclick="confirmUnLock('?action=unlock&id=<?php echo $item['id'];?>')" href='javascript:;'>解锁</a>&nbsp;|&nbsp;
						  <?php
						  }
						  ?>
...
$(function(){
	...
	function delForm(url){
		if(confirm('确定要清空搜索数据吗?')){
			var result = prompt("请输入操作密码：xymf");
			if(result != 'xymf'){
				alert('密码错误!');
				return;
			}
